...
export default function PastBeerForm({ staffNames }: { staffNames: string[] }) {
  ...
  const [state, formAction, isPending] = useActionState(submitPastBeerAction, {});
  ...
  const itemOptions: ItemOption[] = BEER_ITEMS;
  ...
  if (state?.ok) {
    return (
      <div className="card" style={cardStyle}>
        <p style={{ margin: '0 0 12px', color: '#166534', fontWeight: 600 }}>{t('past_entry_beer_saved')}</p>
        ...
        <div style={{ marginTop: 12 }}>
          <Button
            variant="ghost"
            onClick={() => {
              window.location.assign('/admin/add-past-entry?tab=beer');
            }}
          >
    ...
    <div className="card" style={cardStyle}>
      ...
      <form action={formAction} style={{ display: 'grid', gap: 14 }}>
        ...
        <Button type="submit" variant="primary" disabled={isPending || !formOk}>
          {isPending ? t('saving_confirm') : t('past_entry_beer_submit')}
        </Button>
      </form>
    </div>
  );
